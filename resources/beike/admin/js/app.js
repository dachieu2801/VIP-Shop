 ...
 * @LastEditTime  2024-03-26 08:51:01
 */
...
const inputLocaleTranslate = () => {
  $(".translate-btn").click(function () {
    ...
    let $parents = $(this).parents(".input-locale-wrap").length
      ? $(this).parents(".input-locale-wrap")
      : $(this).parents(".col-auto");
    ...
    $http.post("translation", { from, to, text }).then((res) => {
      ...
      res.data.forEach((e) => {
        ...
        if (e.error) {
          ...
          $parents
            .find(".input-" + e.locale)
            .parents(".input-for-group")
            .after(
              '<div class="invalid-feedback translation-error-text mb-1 d-block" style="margin-left: 86px">' +
                e.error +
                "</div>"
            );
        } else {
...
const tinymceInit = () => {
  ...
  tinymce.init({
    ...
    menubar: "",
    toolbar:
      "undo redo | toolbarImageButton | lineheight | bold italic underline strikethrough | forecolor backcolor | fontselect fontsizeselect formatselect | alignleft aligncenter alignright alignjustify | outdent indent | numlist bullist | formatpainter removeformat | charmap emoticons | preview | template link anchor table toolbarImageUrlButton | fullscreen code",
    // contextmenu: "link image imagetools table",
    toolbar_items_size: "small",
    image_caption: true,
    imagetools_toolbar: "",
    toolbar_mode: "wrap",
    //微软雅黑='Microsoft YaHei';黑体=黑体;
    font_formats:
      "Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Georgia=georgia,palatino;Helvetica=helvetica;Times New Roman=times new roman,times;Verdana=verdana,geneva",
          ...
          bk.fileManagerIframe((images) => {
            if (images.length) {
              images.forEach((e) => {
                if (e.mime == "video/mp4") {
                  ed.insertContent(
                    `<video src='${e.path}' controls loop muted class="img-fluid" />`
                  );
                } else {
...
const pageBottomBtns = () => {
  if ($(".page-bottom-btns").length && $(".page-bottom-btns").html().trim()) {
    const contentInfoTop =
      $(".content-info").offset().top + $(".content-info").height();
