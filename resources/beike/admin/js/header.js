...
$(function () {
  ...
  $("#header-search-input").on("keyup", function (key) {
    ...
    if (key.keyCode == 13) {
      const $activeItem = $(
        ".dropdown-search .common-links .dropdown-item.active"
      );
  ...
  $("#header-search-input").on("keydown", function (key) {
    if (key.keyCode == 38 || key.keyCode == 40) {
      ...
      const dropdownSearchLinksTop = $(
        ".dropdown-search .common-links"
      ).offset().top;
      const dropdownSearchHeight = $(".dropdown-search").height() - 34;
      const dropdownSearchTop =
        $(".dropdown-search").offset().top + dropdownSearchHeight;
      const index = $dropdownItem.index(
        $(".dropdown-search .common-links .dropdown-item.active")
      );
      ...
      if (key.keyCode == 38) {
        if (index == "-1" || index == 0) {
          $dropdownItem
            .removeClass("active")
            .eq(searchLinksLength - 1)
            .addClass("active");
          $(".dropdown-search").scrollTop(
            $(".dropdown-search .common-links").height()
          );
        } else {
          $dropdownItem
            .removeClass("active")
          ...
          if (activeTop < dropdownSearchTop - dropdownSearchHeight) {
            $(".dropdown-search").scrollTop(
              activeTop - dropdownSearchLinksTop + 30
            );
          }
        }
      }
      ...
      if (key.keyCode == 40) {
        if (index == "-1" || index == searchLinksLength - 1) {
          ...
          if (activeTop > dropdownSearchTop) {
            $(".dropdown-search").scrollTop(
              activeTop - dropdownSearchLinksTop - dropdownSearchHeight + 40
            );
          }
        }
      }
  ...
  const searchApi = (val) => {
    $http
      .get(`menus?keyword=${val}`, null, { hload: true })
      .then((res) => {
