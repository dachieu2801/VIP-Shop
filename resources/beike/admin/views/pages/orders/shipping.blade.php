    ...
    <div class="container">
        <div id="print-button">
            ...
            <p style="text-align: right;"><button class="btn btn-primary right" type="button" onclick="window.print()"
                    class="printer">{{ __("admin/order.btn_print") }}</button></p>
        </div>
        ...
        <div style="page-break-after: always;">
            ...
            <table class="table">
                <tbody>
                    <tr>
                        <td>
                            <b>{{ __("admin/order.shipping_customer_name") }}: </b>
                            {{ $order['shipping_customer_name'] }}<br />
                            ...
                            <b>{{ __("admin/order.shipping_address") }}: </b>
                            {{ $order['shipping_address_1'] }},{{ $order['shipping_zone'] }},
                            {{ $order['shipping_country'] }}<br />
                            <b>Địa chỉ chi tiết: </b> {{ $order['shipping_address_2'] }}<br />
                            @endif
                            @if($order['receiving_method'] == 'pick_up_items')
                            <b>Địa chỉ cửa hàng: </b> {{ $order['pick_up_address'] }}<br />
                            <b>Tên người nhận hàng: </b> {{ $order['name'] }}<br />
                            <b>Số điện thoại: </b> {{ $order['phone'] }}<br />
                            @endif
                            <b>Giờ nhận hàng: </b>
                            {{$order['receive_time'] ? $order['receive_time'] : $order['pick_up_time']}}<br />
                        </td>
                        <td style="width: 50%;">
                ...
                <tbody>
                    <tr>
                        <td colspan="3">
                            ...
                            <b>{{ __("admin/order.telephone") }}: </b> {{ $order['shipping_telephone'] }}<br />
                            <b>{{ __("admin/order.email") }}: </b> {{ $order['email'] }}<br />
                            <b>{{ __("admin/order.website") }}: </b> <a
                                href="{{ $order['website'] }}">{{ $order['website'] }}</a>
                        </td>
                        </td>
                    </tr>
                </tbody>
