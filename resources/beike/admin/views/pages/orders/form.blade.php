...
<a href="{{ admin_route('orders.shipping.get') }}?order_id={{ $order->id }}" target="_blank"
    class="btn btn-outline-secondary"><i class="bi bi-printer-fill"></i> {{ __('admin/order.btn_print') }}</a>
@endsection
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('admin/common.order') }}</h6>
    </div>
...
<div class="card mb-4">
    ...
    <div class="card-header">
        <h6 class="card-title">{{ __('order.address_info') }}</h6>
    </div>
    <div class="card-body">
        <table class="table">
            <thead class="">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        ...
                        <div>{{ __('address.address_2') }}： {{ $order->shipping_address_2 }}</div>
                        <div>
                            <!-- {{ __('address.address') }}： -->
                    ...
                    <td>
                        ...
                        <div>{{ __('address.address_2') }}：{{ $order->payment_address_2 }}</div>
                        <div>
                            <!-- {{ __('address.address') }}： -->
    ...
    <div class="card-body" id="app">
        <el-form ref="form" :model="form" :rules="rules" label-width="100px">
            ...
            <el-form-item label="{{ __('order.express_company') }}" v-if="form.status == 'shipped'" prop="express_code">
                ...
                <a href="{{ admin_route('settings.index') }}?tab=tab-express-company" target="_blank"
                    class="ms-2">{{ __('common.to_setting') }}</a>
            </el-form-item>
            <el-form-item label="{{ __('order.express_number') }}" v-if="form.status == 'shipped'"
                prop="express_number">
                <el-input class="w-max-500" v-model="form.express_number" size="small" v-if="form.status == 'shipped'"
                    placeholder="{{ __('order.express_number') }}"></el-input>
            </el-form-item>
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('order.product_info') }}</h6>
    </div>
    <div class="card-body">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="wh-60 me-2"><img src="{{ image_resize($product->image) }}"
                                        class="img-fluid max-h-100"></div>{{ $product->name }}
                            </div>
                        </td>
                ...
                <tfoot>
                    ...
                    <tr>
                        ...
                        <td class="text-end"><span
                                class="fw-bold">{{ currency_format($orderTotal->value, $order->currency_code, $order->currency_value) }}</span>
                        </td>
                    </tr>
                    @endforeach
                </tfoot>
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('order.order_comment') }}</h6>
    </div>
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('admin/order.payments_history') }}</h6>
    </div>
    <div class="card-body">
        <div class="table-push">
            <table class="table ">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <!-- <td style="word-wrap: break-word;">{{ $payment->callback }}</td>
                <td>
                  @if ($payment->receipt)
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('order.order_shipments') }}</h6>
    </div>
    <div class="card-body">
        <div class="table-push">
            <table class="table ">
                    ...
                    <tr data-id="{{ $ship->id }}">
                        <td>
                            ...
                            <select class="form-select edit-form express-code d-none"
                                ...
                                <option value="{{ $item['code'] }}"
                                    {{ $ship->express_code == $item['code'] ? 'selected' : '' }}>{{ $item['name'] }}
                                </option>
                                @endforeach
                            </select>
                            @endif
                        ...
                        <td>
                            ...
                            <input type="text" class="form-control edit-form express-number d-none"
                                placeholder="{{ __('order.express_number') }}" value="{{ $ship->express_number }}">
                        </td>
                        <td>
...
<div class="card mb-4">
    <div class="card-header">
        <h6 class="card-title">{{ __('order.action_history') }}</h6>
    </div>
    <div class="card-body">
        <div class="table-push">
            <table class="table ">
...
$('.shipment-tool .btn-primary').click(function() {
    ...
    $http.put(`/orders/{{ $order->id }}/shipments/${id}`, {
        express_code,
        express_name,
        express_number
    }).then((res) => {
