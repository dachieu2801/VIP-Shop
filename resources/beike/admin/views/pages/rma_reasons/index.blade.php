      ...
      <table class="table">
        ...
        <tbody v-if="rmaReasons.length">
          <tr v-for="language, index in rmaReasons" :key="index">
            ...
            <td class="text-end">
              <button class="btn btn-outline-secondary btn-sm mb-1" @click="checkedCreate('edit', index)">{{ __('common.edit') }}</button>
              <button class="btn btn-outline-danger mb-1 btn-sm ml-1" type="button" @click="deleteCustomer(language.id, index)">{{ __('common.delete') }}</button>
            </td>
          </tr>
        </tbody>
