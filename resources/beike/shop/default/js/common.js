...
export default {
    ...
    $(document).ready(() => {
      $http.get("carts/mini", null, { hload: true }).then((res) => {
        ...
        if (!res.data.quantity_all) {
          ...
          $(".cart-badge-quantity")
            .show()
            .html(res.data.quantity_all > 99 ? "99+" : res.data.quantity_all);
        }
      });
    });
  },
  ...
  addCart({ sku_id, quantity = 1, isBuyNow = false }, event, callback) {
    ...
    $http
      .post(
        "/carts",
        { sku_id, quantity, buy_now: isBuyNow },
        { hload: !!event }
      )
      .then((res) => {
        this.getCarts();
        if (!isBuyNow) {
          layer.msg(
            `<div class="d-flex flex-column align-items-center justify-content-center gap-3"><i class="bi bi-check-circle-fill text-success fs-1"></i><p class="fs-2">${res.message}</p></div>`
          );
        }
  ...
  addWishlist(id, event) {
    ...
    if (isWishlist) {
      $btn.html(loadHtml).prop("disabled", true);
      $http
        .delete(`account/wishlist/${isWishlist}`, null, { hload: true })
        .then((res) => {
        ...
        .finally((e) => {
          $btn
            ...
            .prop("disabled", false)
            .find("i.bi")
            .prop("class", "bi bi-heart");
        });
    } else {
      $btn.html(loadHtml).prop("disabled", true);
      $http
        .post("account/wishlist", { product_id: id }, { hload: true })
        .then((res) => {
          layer.msg(res.message);
