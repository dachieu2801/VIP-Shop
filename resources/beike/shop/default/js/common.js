...
export default {
  ...
  addCart({sku_id, quantity = 1, isBuyNow = false}, event, callback) {
    ...
    $http.post('/carts', {sku_id, quantity, buy_now: isBuyNow}, {hload: !!event}).then((res) => {
      this.getCarts();
      if (!isBuyNow) {
        layer.msg(`<div class="d-flex flex-column align-items-center justify-content-center gap-3"><i class="bi bi-check-circle-fill text-success fs-1"></i><p class="fs-2">${res.message}</p></div>`);
      }
