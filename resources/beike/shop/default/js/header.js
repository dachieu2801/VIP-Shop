...
$(function () {
  ...
  $(document).on("click", ".offcanvas-products-delete", function () {
    ...
    $http.delete(`carts/${id}`).then((res) => {
      ...
      if (!res.data.quantity) {
        ...
        $(".cart-badge-quantity")
          .show()
          .html(res.data.quantity > 99 ? "99+" : res.data.quantity);
      }
  ...
  $(document).on("click", ".mobile-open-menu", function () {
    const offcanvasMobileMenu = new bootstrap.Offcanvas(
  ...
  $(document).on(
    "click",
    "#offcanvas-right-cart .product-list .select-wrap",
    function () {
      ...
      $(this)
        .children("i")
        .prop("class", isChecked ? checked : unchecked);

      const checkedProduct = $(
        "#offcanvas-right-cart .offcanvas-right-products i.bi-check-circle-fill"
      ).length;

      $(".offcanvas-footer .all-select i").prop(
        "class",
        productListAll == checkedProduct ? checked : unchecked
      );

      const checkedIds = $("#offcanvas-right-cart .product-list")
        .map(function () {
      ...
      $http
        .post(
          `/carts/${isChecked ? "select" : "unselect"}`,
          {
            cart_ids: [cartId],
          },
          {
  ...
  $(document).on("click", "#offcanvas-right-cart .all-select", function () {
    ...
    $("#offcanvas-right-cart .product-list")
      .find(".select-wrap i")
      .prop("class", isChecked ? checked : unchecked);
  ...
  (function () {
    ...
    $(document).click(function (e) {
      if (
        !$(e.target).closest(".account-sides-info").length &&
        !$(e.target).closest(".mb-account-icon").length
      ) {
        $(".account-sides-wrap").removeClass("active");
      }
    });
  ...
  (function () {
    const pbHeader =
      $(window).width() > 768 ? $(".header-content") : $(".header-mobile");
    ...
    $(window).scroll(function () {
      ...
      if (scrollTop > headerContentTop) {
        ...
        if (!$(".header-content-placeholder").length)
          pbHeader.before(
            '<div class="header-content-placeholder" style="height: ' +
              headerContentHeight +
              'px"></div>'
          );
      } else {
