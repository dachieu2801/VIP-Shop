...
//        return DB::select($sql);
        ...
        $sql = "SELECT cp.category_id AS id,
               TRIM(LOWER(GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '{$separator}'))) AS name,
               c1.parent_id,
               c1.position
        FROM category_paths cp
        LEFT JOIN categories c1 ON (cp.category_id = c1.id)
        LEFT JOIN categories c2 ON (cp.path_id = c2.id)
        LEFT JOIN category_descriptions cd1 ON (cp.path_id = cd1.category_id)
        ...
        $sql .= ' GROUP BY cp.category_id, c1.parent_id, c1.position
          ORDER BY c1.position ASC, name ASC';
