...
class MarketingService
{
    ...
    public function getPlugin($pluginCode): mixed
    {
        ...
        if (empty($plugin)) {
            throw new NotFoundHttpException('Plug-in không tồn tại hoặc đã bị xóa khỏi kệ');
        }
    ...
    public function getPluginServiceOrder($pluginServiceOrderId): mixed
    {
        ...
        if (empty($plugin)) {
            throw new NotFoundHttpException('Đơn đặt hàng dịch vụ plug-in không tồn tại hoặc đã bị xóa khỏi kệ');
        }
