...
class AccountService
{
    ...
    /**
     *  xác minh là chính xác và thay đổi mật khẩu thành mật khẩu mới
     * @param        $code
     * @param        $account
     * @param        $password
    ...
    public static function verifyAndChangePassword($code, $account, $password, string $type = 'email')
    {
        ...
        if ($type == 'email') {
            ...
            throw new \Exception('Lấy lại mật khẩu qua số điện thoại di động hiện không được hỗ trợ.');
        } else {
            throw new \Exception('Loại lấy lại mật khẩu sai');
        }
