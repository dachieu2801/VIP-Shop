#  **github https://github.com/hatran98/BeikeShop_V1** 
...
2.  `composer update` hoặc `composer update --ignore-platform-reqs`
3. Tạo file `.env` với nội dung ở trong file `.env.example`
4. `npm install`（cài đặt node 16+）,`npm run prod` biên dịch các tệp JavaScript và CSS
5. Đặt `public` trong thư mục dự án vào thư mục gốc của trang web
6. Truy cập trang web thông qua trình duyệt của bạn và làm theo lời nhắc để hoàn tất cài đặt.
7. Nếu bạn cần nâng cấp, vui lòng chạy nó trong thư mục gốc của trang web phía máy chủ.`git pull && composer install && php artisan migrate`
...
#  **请移步到 https://github.com/hatran98/BeikeShop_V1** 
