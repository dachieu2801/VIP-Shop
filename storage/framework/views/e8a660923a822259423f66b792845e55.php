<div id="checkout-address-app" v-cloak>
  <div class="checkout-black" v-if="shippingRequired">
    <div class="checkout-title">
      <div class="d-flex">
        <h5 class="mb-0 me-4"><?php echo e(__('shop/checkout.address')); ?></h5>
        <el-checkbox v-model="same_as_shipping_address" v-if="source.addresses.length || source.guest_shipping_address"><?php echo e(__('shop/checkout.same_as_shipping_address')); ?>
              ...
              <div class="address-bottom">
                <div>
                  <span class="badge bg-success"
                    v-if="form.shipping_address_id == address.id"><?php echo e(__('shop/checkout.chosen')); ?></span>
                </div>
                <button type="button" class="btn btn-outline-secondary btn-sm"
                  @click.stop="editAddress(index, 'shipping_address_id')"><?php echo e(__('shop/checkout.edit')); ?></button>
              </div>
            </div>
          </div>
          <div class="col-lg-6 col-12" v-if="!isAllAddress">
            <div class="item address-right">
              <button class="btn btn-outline-dark w-100 mb-md-3" v-if="source.addresses.length > 1"
                @click="isAllAddress = true"><?php echo e(__('shop/checkout.choose_another_address')); ?></button>
              <button class="btn btn-outline-dark w-100" @click="editAddress(null, 'shipping_address_id')"><i
                  class="bi bi-plus-square-dotted"></i> <?php echo e(__('shop/checkout.add_new_address')); ?></button>
            </div>
          </div>
        </template>
        <template v-else>
          <div class="col-lg-6 col-12" v-if="source.guest_shipping_address">
            <div class="item active">
              ...
              <div class="address-bottom">
                <div>
                  <span class="badge bg-success"><?php echo e(__('shop/checkout.chosen')); ?></span>
                </div>
                <button type="button" class="btn btn-outline-secondary btn-sm"
                  @click.stop="editAddress(null, 'guest_shipping_address')"><?php echo e(__('shop/checkout.edit')); ?></button>
              </div>
            </div>
          </div>
          <div class="col-lg-6 col-12" v-if="!source.guest_shipping_address">
  ...
  <div class="checkout-black" v-if='!this.shippingRequired || !same_as_shipping_address'>
    <div class="checkout-title">
      <div class="d-flex">
        <h5 class="mb-0 me-4"><?php echo e(__('shop/checkout.payment_address')); ?></h5>
      </div>
      ...
      <div class="row">
        <template v-if="source.isLogin">
          <div class="col-lg-6 col-12" v-for="address, index in source.addresses" :key="index"
            ...
            <div :class="['item', address.id == form.payment_address_id ? 'active' : '']"
              ...
              <div class="address-bottom">
                <div>
                  <span class="badge bg-success"
                    v-if="form.payment_address_id == address.id"><?php echo e(__('shop/checkout.chosen')); ?></span>
                </div>
                <a class="javascript:void(0)"
                  @click.stop="editAddress(index, 'payment_address_id')"><?php echo e(__('shop/checkout.edit')); ?></a>
              </div>
            </div>
          </div>
          <div class="col-lg-6 col-12" v-if="!isAllAddressPayment">
            <div class="item address-right">
              <button class="btn btn-outline-dark w-100 mb-md-3" v-if="source.addresses.length > 1"
                @click="isAllAddressPayment = true"><?php echo e(__('shop/checkout.choose_another_address')); ?></button>
          ...
          <div class="col-lg-6 col-12" v-if="source.guest_payment_address">
            <div class="item active">
              ...
              <div class="address-bottom">
                ...
                <a class="javascript:void(0)"
                  @click.stop="editAddress(null, 'guest_payment_address')"><?php echo e(__('shop/checkout.edit')); ?></a>
              </div>
            </div>
          </div>
          <div class="col-lg-6 col-12" v-if="!source.guest_payment_address">
...
<script>
  var checkoutAddressApp = new Vue({
    ...
    data: {
      form: {
        shipping_address_id: <?php echo json_encode($current['shipping_address_id'], 15, 512) ?>,
        payment_address_id: <?php echo json_encode($current['payment_address_id'], 15, 512) ?>,
      },

      isAllAddress: false,
      isAllAddressPayment: false,
      shippingRequired: <?php echo json_encode($shipping_require ?? true, 15, 512) ?>,

      source: {
        addresses: <?php echo json_encode($addresses ?? [], 15, 512) ?>,
        guest_shipping_address: <?php echo json_encode($current['guest_shipping_address'] ?? null, 15, 512) ?>,
        guest_payment_address: <?php echo json_encode($current['guest_payment_address'] ?? null, 15, 512) ?>,
      ...
      onAddressDialogChange(form) {
        ...
        if (!this.source.isLogin) {
          ...
          $http.put('/checkout', data).then((res) => {
            ...
            this.$message.success('<?php echo e(__('common.edit_success')); ?>');
...
<?php $__env->stopPush(); ?>
<?php /**PATH D:\team-free-lance\shop-freelance\themes\default/checkout/_address.blade.php ENDPATH**/ ?>