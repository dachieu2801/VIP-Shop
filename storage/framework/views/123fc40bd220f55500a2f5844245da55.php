<template id="link-selector">
  <div class="link-selector-wrap">
    <div class="title" v-if="isTitle"><i class="el-icon-link"></i><?php echo e(__('admin/builder.modules_choose_link')); ?></div>
    <div class="selector-type" @blur="selectorContentShow = false" tabindex="1">
      <div class="title" v-if="link.type != 'custom' ? value.value == '' : ''" @click="selectorContentShow = !selectorContentShow"><?php echo e(__('admin/builder.modules_choose_link')); ?></div>
      ...
      <div :class="'selector-content ' + (selectorContentShow ? 'active' : '')">
        <div @click="selectorType()"><?php echo e(__('admin/builder.text_no')); ?></div>
    ...
    <el-dialog
      ...
      <div slot="title" class="link-dialog-header">
        <div class="title"><?php echo e(__('admin/builder.modules_choose')); ?> {{ dialogTitle }}</div>
        <div class="input-with-select" v-if="link.type != 'custom'">
          <input type="text" placeholder="<?php echo e(__('admin/builder.modules_keywords_search')); ?>" v-model="keyword" @keyup.enter="searchProduct" class="form-control">
          <el-button  @click="searchProduct"><i class="el-icon-search"></i> <?php echo e(__('admin/builder.text_search')); ?></el-button>
        </div>
      </div>
      <div class="link-dialog-content">
        <div class="product-search">
          <div class="link-top-new">
            <span><?php echo e(__('admin/builder.text_is_newpage')); ?></span>
            <el-switch :width="36" @change="linksNewBack" v-model="link.new_window"></el-switch>
          </div>

          <a :href="linkTypeAdmin" target="_blank" v-if="link.type != 'custom' && link.type != 'static'"><?php echo e(__('admin/builder.text_manage')); ?> {{ dialogTitle }}</a>
        </div>

        <div class="link-text" v-if="isCustomName">
          <div class="module-edit-group" style="margin-bottom: 10px;">
            <div class="module-edit-title"><?php echo e(__('admin/builder.custom_name')); ?></div>
            <text-i18n v-model="link.text"></text-i18n>
          </div>
        </div>
        <template v-if="link.type == 'custom'">
          <div class="linkDialog-custom">
            <el-input v-model="link.value" placeholder="<?php echo e(__('admin/builder.text_enter_link')); ?>"></el-input>
          </div>
        </template>
          ...
          <div class="product-info" v-loading="loading">
            <template v-if="linkDialog.data.length">
              <div class="product-info-title">
                <span><?php echo e(__('admin/builder.modules_content')); ?></span>
                <span><?php echo e(__('admin/builder.text_states')); ?></span>
              </div>
              ...
              <ul class="product-list">
                <li v-for="(product, index) in linkDialog.data" @click="product.status ? link.value = product.id : false" :class="!product.status ? 'no-status' : ''">
                  ...
                  <div :class="'right ' + (product.status ? 'ok' : 'no')">
                    <template v-if="product.status"><?php echo e(__('admin/builder.text_enable')); ?></template>
                    <template v-else><?php echo e(__('admin/builder.text_disable')); ?></template>
                  </div>
                </li>
              </ul>
            </template>
            <div class="product-info-no" v-if="!linkDialog.data.length && loading === false">
              ...
              <div class="no-text"><?php echo e(__('admin/builder.text_no_data')); ?>, <a :href="linkTypeAdmin" target="_blank"><?php echo e(__('admin/builder.text_to_add')); ?>{{ dialogTitle }}</a></div>
            </div>
          </div>
        </template>
      </div>
      <div slot="footer" class="link-dialog-footer">
        <el-button type="primary" @click="linkDialogConfirm"><?php echo e(__('admin/builder.text_sure')); ?></el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script type="text/javascript">
  Vue.component('link-selector', {
    ...
    data: function () {
      return {
        types: [
          {type: 'product', label: '<?php echo e(__('admin/builder.modules_product')); ?>'},
          {type: 'category', label: '<?php echo e(__('admin/builder.text_category')); ?>'},
          {type: 'page', label: '<?php echo e(__('admin/builder.text_information')); ?>'},
          {type: 'page_category', label: '<?php echo e(__('admin/builder.page_category')); ?>'},
          {type: 'brand', label: '<?php echo e(__('admin/builder.text_manufacturer')); ?>'},
          {type: 'static',label: '<?php echo e(__('admin/builder.text_static')); ?>'},
          {type: 'custom',label: '<?php echo e(__('admin/builder.text_custom')); ?>'}
        ],
        static: [
          {name: '<?php echo e(__('shop/account.index')); ?>', value: 'account.index'},
          {name: '<?php echo e(__('shop/account/wishlist.index')); ?>', value: 'account.wishlist.index'},
          {name: '<?php echo e(__('shop/account/order.index')); ?>', value: 'account.order.index'},
          // {name: '最新商品', value: 'account.index'},
          {name: '<?php echo e(__('shop/brands.index')); ?>', value: 'brands.index'},
        ],
        link: null,
        keyword: '',
        name: '',
    ...
    computed: {
      ...
      linkTypeAdmin: function () {
        ...
        switch(this.link.type) {
          case 'product':
            url = '<?php echo e(admin_route('products.index')); ?>';
            break;
          case 'category':
            url = '<?php echo e(admin_route('categories.index')); ?>';
            break;
          case 'brand':
            url = '<?php echo e(admin_route('brands.index')); ?>';
            break;
          case 'page':
            url = '<?php echo e(admin_route('pages.index')); ?>';
            break;
          case 'page_category':
            url = '<?php echo e(admin_route('page_categories.index')); ?>';
            break;
          default:
            null;
        }
      ...
      updateData() {
        ...
        $http.get(url, null, {hload: true, hmsg: true}).then((res) => {
          if (res.data) {
            ...
            self.name = '<?php echo e(__('admin/builder.text_no_data')); ?>';
          }
        }).catch(() => {
          self.name = '<?php echo e(__('admin/builder.text_no_data')); ?>';
        }).finally(() => {
...
<?php /**PATH D:\team-free-lance\shop-freelance\resources\/beike/admin/views/pages/design/builder/component/link_selector.blade.php ENDPATH**/ ?>