  <template id="pb-image-selector">
    <div class="pb-image-selector">
      <el-tabs v-if="isLanguage" @tab-click="tabClick" value="language-<?php echo e(locale()); ?>" :stretch="languages.length > 5 ? true : false" type="card" :class="languages.length <= 1 ? 'languages-a' : ''">
        <el-tab-pane v-for="(item, index) in languages" :key="index" :label="item.name" :name="'language-' + item.code">
          ...
          <div class="i18n-inner">
            ...
            <div class="btns">
              <el-button type="primary" size="mini" plain @click="selectButtonClicked"><?php echo e(__('admin/builder.modules_choose')); ?></el-button>
              <el-button size="mini" plain style="margin-left: 4px;" @click="removeImage"><?php echo e(__('admin/builder.text_delete')); ?></el-button>
            </div>
          ...
          bk.fileManagerIframe(images => {
            ...
            if (this.isLanguage) {
              if(images[0].path.charAt(0)==='/'){
                this.src[this.tabActiveId] = images[0].path.slice(1, images[0].path.length)
              }
            } else {
              if(images[0].path.charAt(0)==='/'){
                this.src = images[0].path.slice(1, images[0].path.length)
              }
            }
          })
        }
...
<?php /**PATH D:\team-free-lance\shop-freelance\resources\/beike/admin/views/pages/design/builder/component/image_selector.blade.php ENDPATH**/ ?>