...
<head>
  ...
  <script src="<?php echo e(asset('vendor/vue/2.7/vue.js')); ?>"></script>
  ...
  <script src="<?php echo e(asset('vendor/vue/batch_select.js')); ?>"></script>
  <link href="<?php echo e(mix('/build/beike/admin/css/bootstrap.css')); ?>" rel="stylesheet">
  <link rel="stylesheet" href="<?php echo e(asset('vendor/element-ui/index-blue.css')); ?>">
...
<body class="page-filemanager">
  <div class="filemanager-wrap" id="filemanager-wrap-app" v-cloak ref="splitPane">
    ...
    <div class="filemanager-navbar" :style="'width:' + paneLengthValue">
      <el-tree
        ...
        <div class="custom-tree-node" slot-scope="{ node, data }" :path="data.path" :name="data.name">
          ...
          <div class="right">
            <el-tooltip class="item file-download" effect="dark" content="<?php echo e(__('admin/file_manager.download')); ?>"
            placement="top">
            <span @click.stop="() => {openInputBox('download', node, data)}"><i
              class="el-icon-download"></i></span>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="<?php echo e(__('admin/file_manager.create_folder')); ?>"
              placement="top">
              <span @click.stop="() => {openInputBox('addFolder', node, data)}"><i
                  class="el-icon-circle-plus-outline"></i></span>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="<?php echo e(__('admin/file_manager.rename')); ?>" placement="top">
              <span v-if="node.level != 1" @click.stop="() => {openInputBox('renameFolder', node, data)}"><i
                  class="el-icon-edit"></i></span>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="<?php echo e(__('common.delete')); ?>" placement="top">
    ...
    <div class="filemanager-content" v-loading="loading" element-loading-background="rgba(255, 255, 255, 0.5)">
      <div class="content-head">
        <div class="left d-lg-flex">
          <el-button class="me-4 mb-1 mb-lg-0" size="small" icon="el-icon-check" type="primary" @click="fileChecked"
            :disabled="!!!selectImageIndex.length"><?php echo e(__('admin/builder.modules_choose')); ?></el-button>
          <el-link :underline="false" :disabled="!!!selectImageIndex.length" icon="el-icon-view" @click="viewImages"><?php echo e(__('common.view')); ?></el-link>
          <el-link :underline="false" :disabled="!!!selectImageIndex.length" icon="el-icon-document-copy" @click="copyLink"><?php echo e(__('admin/file_manager.copy_link')); ?></el-link>
          <el-link :underline="false" :disabled="!!!selectImageIndex.length" @click="deleteFile" icon="el-icon-delete">
            <?php echo e(__('common.delete')); ?></el-link>
          <el-link :underline="false" :disabled="selectImageIndex.length == 1 ? false : true"
            @click="openInputBox('image')" icon="el-icon-edit"><?php echo e(__('admin/file_manager.rename')); ?></el-link>
          <el-link v-if="isMultiple" :underline="false" :disabled="!!!images.length && !!!selectImageIndex.length" @click="selectAll()"
            icon="el-icon-finished"><?php echo e(__('common.select_all')); ?></el-link>
           <?php
                ...
                $output = \Hook::getHook("admin.file_manager.content.head.btns.after",["data"=>$__definedVars],function($data) { return null; });
                if ($output)
                echo $output;
                ?>
        </div>
        <div class="right">
           <?php
                ...
                $output = \Hook::getHook("admin.file_manager.content_head.right",["data"=>$__definedVars],function($data) { return null; });
                if ($output)
                echo $output;
                ?>
          <el-popover placement="bottom" width="360" class="me-1" trigger="manual" v-model="filterKeywordVisible" ref="keyword-popover">
            <div class="d-flex keyword-popover-area">
              <el-input placeholder="<?php echo e(__('common.input')); ?>" v-model="filterKeyword" class="input-with-select" @keyup.enter.native="searchFile">
          ...
          <el-popover placement="bottom" width="260" class="me-1" trigger="click">
            <div class="text-center mb-3 fw-bold"><?php echo e(__('admin/file_manager.file_sorting')); ?></div>
            <div class="mb-3">
              <div class="mb-2"><?php echo e(__('admin/file_manager.text_type')); ?></div>
              <el-radio-group v-model="filter.sort" size="small">
                <el-radio-button label="created"><?php echo e(__('admin/file_manager.text_created')); ?></el-radio-button>
                <el-radio-button label="name"><?php echo e(__('admin/file_manager.file_name')); ?></el-radio-button>
              </el-radio-group>
            </div>

            <div class="mb-3">
              <div class="mb-2"><?php echo e(__('admin/file_manager.to_sort')); ?></div>
              <el-radio-group v-model="filter.order" size="small">
                <el-radio-button label="desc"><?php echo e(__('admin/file_manager.text_desc')); ?></el-radio-button>
                <el-radio-button label="asc"><?php echo e(__('admin/file_manager.text_asc')); ?></el-radio-button>
              </el-radio-group>
            </div>
            <el-button slot="reference" size="small" plain type="primary" icon="el-icon-s-operation"></el-button>
          </el-popover>
          <el-button size="small" plain type="primary" @click="openUploadFile" icon="el-icon-upload2"><?php echo e(__('admin/file_manager.upload_files')); ?></el-button>
        </div>
      </div>
      ...
      <el-empty v-else description="<?php echo e(__('admin/file_manager.no_file')); ?>"></el-empty>
    ...
    <?php else: ?>
    <div class="text-center mt-5 w-100 fs-4"><?php echo e(__('admin/file_manager.show_pc')); ?></div>
    <?php endif; ?>

    <el-dialog title="<?php echo e(__('admin/file_manager.upload_files')); ?>" top="12vh" :visible.sync="uploadFileDialog.show"
      ...
      <el-upload class="photos-upload" target="photos-upload" id="photos-upload"
        element-loading-text="<?php echo e(__('admin/file_manager.image_uploading')); ?>..."
        ...
        <div class="el-upload__text"><?php echo e(__('admin/file_manager.click_upload')); ?></div>
      </el-upload>
      <div class="upload-image">
        <div v-for="image, index in uploadFileDialog.images" :key="index" class="list">
          <div class="info">
            ...
            <div class="status">
              <span v-if="image.status == 'complete'" class="text-success"><?php echo e(__('admin/file_manager.finish')); ?></span>
              <span v-else-if="image.status == 'fail'" class="text-danger"><?php echo e(__('admin/file_manager.upload_fail')); ?></span>
              <span v-else><?php echo e(__('admin/file_manager.uploading')); ?></span>
            </div>
          </div>
    ...
    <div class="drop_file_hint d-none"><?php echo __('admin/file_manager.drop_file_hint'); ?></div>
    <div class="drop_folder_hint d-none"><?php echo __('admin/file_manager.drop_folder_hint'); ?></div>
  </div>

  <?php echo $__env->yieldPushContent('admin.file_manager.footer'); ?>
  ...
  <script>
    ...
    var app = new Vue({
      ...
      data: {
        ...
        triggerLength: 10,
        isShift: false,
        mime: <?php echo json_encode(request('mime'), 15, 512) ?>,
        isMultiple: <?php echo e(request('is_multiple', true)); ?>,
        loading: false,
        isBatchSelect: false,
        selectImageIndex: [],
        filterKeyword: '',
        ...
        treeData: [{
          name: '<?php echo e(__('admin/file_manager.picture_space')); ?>',
          path: '/',
          children: []
        }],
        ...
        image_page: 1,
        per_page: 20,
        <?php echo $__env->yieldPushContent('admin.file_manager.vue.data'); ?>
      },
      computed: {
        paneLengthValue() {
      ...
      methods: {
        ...
        handleDrop(draggingNode, dropNode, dropType, ev) {
          ...
          this.$confirm($('.drop_folder_hint').html(),"<?php echo e(__('common.text_hint')); ?>", {
        ...
        imgMove(path, name, selectImageIndex) {
          ...
          this.$confirm($('.drop_file_hint').html(),"<?php echo e(__('common.text_hint')); ?>", {
            dangerouslyUseHTMLString:true,
            type: "warning"
          }).then(() => {
        ...
        fileChecked() {
          ...
          if (this.mime) {
            if (this.mime == 'image' && typedFiles.some(e => !e.mime.startsWith('image'))) {
              layer.msg('<?php echo e(__('admin/file_manager.verify_select_image')); ?>', () => {});
              return;
            }

            if (this.mime == 'video' && typedFiles.some(e => !e.mime.startsWith('video'))) {
              layer.msg('<?php echo e(__('admin/file_manager.verify_select_video')); ?>', () => {});
              return;
            }
          }
        ...
        deleteFile() {
          this.$confirm('<?php echo e(__('admin/file_manager.confirm_delete_file')); ?>', '<?php echo e(__('common.text_hint')); ?>', {
            type: 'warning'
          }).then(() => {
        ...
        deleteFolder(node, data) {
          if (data.path) {
            this.$confirm('<?php echo e(__('admin/file_manager.confirm_delete_folder')); ?>', '<?php echo e(__('common.text_hint')); ?>', {
              type: 'warning'
            }).then(() => {
        ...
        copyLink() {
          ...
          layer.msg('<?php echo e(__('admin/file_manager.copy_success')); ?>')
        },
        ...
        openInputBox(type, node, data) {
          ...
          this.$prompt('', type == 'addFolder' ? '<?php echo e(__('admin/file_manager.new_folder')); ?>' : '<?php echo e(__('admin/file_manager.rename')); ?>', {
            confirmButtonText: '<?php echo e(__('common.confirm')); ?>',
            cancelButtonText: '<?php echo e(__('common.cancel')); ?>',
            inputPattern: /^.+$/,
            closeOnClickModal: false,
            inputValue: type == 'image' ? fileName : (type == 'renameFolder' ? data.name : '<?php echo e(__('admin/file_manager.new_folder')); ?>'),
            inputErrorMessage: '<?php echo e(__('admin/file_manager.can_empty')); ?>'
          }).then(({
            value
          }) => {
            if (type == 'addFolder') {
        ...
        <?php echo $__env->yieldPushContent('admin.file_manager.vue.method'); ?>
      },
    })
...
<?php /**PATH G:\workspace\shop-freelance\resources\/beike/admin/views/pages/file_manager/index.blade.php ENDPATH**/ ?>