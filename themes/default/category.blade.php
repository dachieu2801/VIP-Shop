...
@section('title', $category->description->meta_title ?: system_setting('base.meta_title', 'BeikeShop là một hệ thống thương mại điện tử xuyên biên giới mã nguồn mở và dễ sử dụng') .' - '. $category->description->name)
...
{{--          @endif--}}
          <div class="row {{ request('style_list') == 'list' ? 'product-list-wrap' : ''}}">
            @foreach ($products_format as $product)
              <div class="{{ !request('style_list') || request('style_list') == 'grid' ? 'product-grid col-6 col-md-4 mb-3' : 'col-12'}}">
